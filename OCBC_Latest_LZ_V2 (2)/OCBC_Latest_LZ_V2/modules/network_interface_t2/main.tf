...
resource "azurerm_network_interface" "nic_t2" {
  count = length(var.subnet_ids)
  name                = lookup(var.nic_properties[count.index], "name")
  # provider = azurerm.NameOfSubscription1
...
resource "azurerm_network_security_group" "nsg_t2" {
  name             = var.nsg_name
  location            = var.location
  resource_group_name = var.nic_rg_name
...
resource "azurerm_subnet_network_security_group_association" "example_t2" {
  count = length(var.subnet_ids)
  subnet_id                     = var.subnet_ids[count.index]
  # network_interface_id      = azurerm_network_interface.nic.0.id
  network_security_group_id = azurerm_network_security_group.nsg_t2.id
}